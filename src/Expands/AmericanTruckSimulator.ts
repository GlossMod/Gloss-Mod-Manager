...
async function handleMod(
    ...
    for (const item of mod.modFiles) {
        if ((await FileHandler.getFileExtension(item)).toLowerCase() !== "scs") {
            continue;
        }
...
export const supportedGames = async () =>
    ({
        ...
        steamAppID: 270880,
        curseforge: 4819,
        installdir: await join("American Truck Simulator", "bin", "win_x64"),
        ...
        startExe: [
            {
                name: "Steam 启动",
                cmd: "steam://rungameid/270880",
            },
            {
                name: "直接启动",
                exePath: await join("bin", "win_x64", "amtrucks.exe"),
            },
        ],
        ...
        gameCoverImg:
            "https://assets-mod.3dmgame.com/static/upload/game/52.jpg",
        modType: [
            {
                id: 1,
                name: "scs",
        ...
        async checkModType(mod) {
            ...
            for (const item of mod.modFiles) {
                if ((await FileHandler.getFileExtension(item)).toLowerCase() === "scs") scs = true;
